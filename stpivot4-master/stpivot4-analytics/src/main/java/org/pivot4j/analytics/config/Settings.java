...
public class Settings {
	...
	public static final String CONFIG_FILE = "stpivot4.config";

	public static final String APPLICATION_HOME = "stpivot4.home";
	...
	protected void initialize() {
		...
		try {
			...
			if (configPath == null || stage == ProjectStage.UnitTest) {
				configPath = path + File.separator + "stpivot4-config.xml";

				File configFile = new File(configPath);

				if (!configFile.exists() || stage == ProjectStage.UnitTest) {
					String defaultConfig = "/WEB-INF/stpivot4-config.xml";
		...
		if (logger.isInfoEnabled()) {
			logger.info("STPivot4 Analytics has been initialized successfully.");
		}
