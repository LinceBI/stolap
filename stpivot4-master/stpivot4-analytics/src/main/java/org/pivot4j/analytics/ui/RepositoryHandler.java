...
public class RepositoryHandler implements ViewStateListener, Serializable {
	...
	public void saveAs() {
		...
		if (reportName.toLowerCase().endsWith(".stpivot4")) {
			reportName = reportName.substring(0, reportName.length() - 8);
		}

		String fileName = reportName + ".stpivot4";
	...
	public void open() {
		...
		if (name.toLowerCase().endsWith(".stpivot4")) {
	...
	public void suggestNewName() {
		...
		try {
			...
			for (ReportFile child : children) {
				...
				if (childName.toLowerCase().endsWith(".stpivot4")) {
