...
public class Settings {
	...
	public static final String CONFIG_FILE = "stolap.config";

	public static final String APPLICATION_HOME = "stolap.home";
	...
	protected void initialize() {
		...
		try {
			...
			if (configPath == null || stage == ProjectStage.UnitTest) {
				configPath = path + File.separator + "stolap-config.xml";

				File configFile = new File(configPath);

				if (!configFile.exists() || stage == ProjectStage.UnitTest) {
					String defaultConfig = "/WEB-INF/stolap-config.xml";
		...
		if (logger.isInfoEnabled()) {
			logger.info("STOlap Analytics has been initialized successfully.");
		}
